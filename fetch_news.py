...
def write_page(filename, html):
    if 'G-J29S2PGT2W' not in html:
        ga = ('<script async src="https://www.googletagmanager.com/gtag/js?id=G-J29S2PGT2W"></script>'
...
def generate_sitemap(all_articles, n_main_pages):
    ...
    urls  = []
    urls.append((base + '/', today, 'daily', '1.0'))
    urls.append((base + '/news.html', today, 'daily', '0.9'))
    for n in range(2, n_main_pages + 1):
        urls.append((base + '/news-' + str(n) + '.html', today, 'daily', '0.8'))
    for cat in ['tool', 'money', 'course', 'prompt', 'news']:
        urls.append((base + '/news-cat-' + cat + '.html', today, 'weekly', '0.7'))
    ...
    print('  sitemap.xml generated (' + str(len(urls)) + ' URLs)')
